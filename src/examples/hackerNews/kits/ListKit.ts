...
export const ListKit = new KitBuilder({
	 ...
	 * @returns
	 */
	async split(inputs: InputValues): Promise<SplitOutput> {
		const {
			input,
			delimiter,
			split_by_each,
		...
		}: SplitInput = inputs as SplitInput;
		let values: (string)[];
        ...
        let res:({text: string; delimiter: string;}) [] = []

        values.forEach(function(value){
           const tmp = {"text": value, "delimiter": delimiter.delimiter as string}
           
           res.push(tmp)
        })

		return Promise.resolve({ values: res });
	},
});
